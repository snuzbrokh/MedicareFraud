...
shinyServer(function(input, output, session) {
    ...
    output$actor1_plot = renderPlot({
        ...
        if (grepl('PRV',V(bn.actor1)$name[1], fixed=TRUE)){
            plotActor(bn.actor1, 
                      paste0("Projected Provider Network: ",input$state_select,
                             ', County: ',input$county_select),
                      actor='provider',
                      layout = 'tree')
        } else if (grepl('PHY',V(bn.actor1)$name[1], fixed=TRUE)){
            plotActor(bn.actor1, 
                      paste0("Projected Doctor Net: ",input$state_select,
                             ', County: ',input$county_select),
                      actor='doctor',
                      layout = 'tree')
        } else {
            plotActor(bn.actor1, 
                      paste0("Projected Patient Net: ",input$state_select,
                             ', County: ',input$county_select),
                      actor='patient',
                      layout = 'mds')
        }
    ...
    output$actor2_plot = renderPlot({
        ...
        if (grepl('PRV',V(bn.actor2)$name[1], fixed=TRUE)){
            ...
            plotActor(bn.actor2, 
                      paste0("Projected Doctor Network: ",input$state_select,
                             ', County: ',input$county_select),
                      actor='doctor',
                      layout = 'tree')
        } else {
            plotActor(bn.actor2, 
                      paste0("Projected Patient Network: ",input$state_select,
                             ', County: ',input$county_select),
                      actor='patient',
                      layout = 'mds')
        }
